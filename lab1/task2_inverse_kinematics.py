...
class MetaData:
    ...
    def get_path_from_root_to_end(self):
        ...
        输出：
            path: 各个关节的索引, from root to end.
            path_name: 各个关节的名字, from root to end.
            path1: joint indices from end to RootJoint.
            path2: joint indices from root to RootJoint.
        Note: 
            如果root_joint在脚，而end_joint在手，那么此路径会路过RootJoint节点。
            在这个例子下path2返回从脚到根节点的路径，path1返回从根节点到手的路径。*I don't think this is true in general.*
            你可能会需要这两个输出。
        """
